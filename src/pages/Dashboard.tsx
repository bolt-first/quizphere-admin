...
function Dashboard() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      ...
      try {
        const response = await fetch(`http://localhost:8001/admin/${adminId}/users`);
    ...
    const fetchQuizzes = async () => {
      ...
      try {
        const response = await fetch(`http://localhost:8001/quizzes/admin/${adminId}`);
        if (response.status === 404) {
          setQuizzes([]);
          return;
        }
    ...
    const fetchQuizResults = async () => {
      ...
      try {
        const response = await fetch(`http://localhost:8001/admin/${adminId}/quiz-results`);
        if (response.status === 404) {
          setQuizResults([]);
          return;
        }
